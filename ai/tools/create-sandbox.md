...
- Execute shell commands with `Run Command` and `Wait Command` (backed by Trigger.dev tasks)
- Access running servers through public URLs using `Get Sandbox URL`

Each sandbox mimics a real-world development environment and supports rapid iteration and testing without polluting the local system. The base system is provided by E2B (Debian-based) with common build tooling available; you can install additional packages as needed.

You can install additional packages using the native package manager. Avoid port 8080 (reserved internally); pick a different port when exposing previews.
