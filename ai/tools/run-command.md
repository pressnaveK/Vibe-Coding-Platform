Use this tool to run a command inside an existing E2B sandbox. Execution is orchestrated through Trigger.dev tasks so logs stream reliably back to the UI. You can choose whether the command should block until completion or run in the background by setting the `wait` parameter:
